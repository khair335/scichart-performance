      ...
      try {
        ...
        if (typeof raw === 'string') {
          ...
          if (t === 'init_begin') {
            ...
            const serverRestarted = this.lastSeq > 0 && (this.minSeq < this.lastSeq || this.wmSeq < this.lastSeq);
            
            // CRITICAL: Check if requested resume point is beyond server's watermark
            // This happens when:
            // 1. Client has persisted lastSeq from previous session (e.g., 8790)
            // 2. Server's watermark is lower than requested seq (e.g., wmSeq=8789, requested=8790)
            // 3. Server will send NO history data because start > wmSeq
            //
            // This applies to BOTH static and live feeds:
            // - Static feed: Server has fixed watermark, no more data will come
            // - Live feed: Server will continue generating, but client will miss data between wmSeq and requested
            // In both cases, reconnecting from seq=1 (or minSeq) ensures we get all available data
            const start = Math.max(this.resumeFromRequested, this.minSeq);
            const requestedBeyondWatermark = start > this.wmSeq;
            
            if (serverRestarted || requestedBeyondWatermark) {
              const reason = serverRestarted 
                ? 'server restart detected' 
                : `requested seq ${this.resumeFromRequested} beyond watermark ${this.wmSeq}`;
              console.log(`[WsFeedClient] 🔄 ${reason}: minSeq=${this.minSeq}, wmSeq=${this.wmSeq}, lastSeq=${this.lastSeq}, requested=${this.resumeFromRequested}, start=${start}, resetting and reconnecting`);
              
              // Clear the persisted lastSeq so next connection starts from seq=1
              this.lastSeq = 0;
              ...
              // Close current connection and reconnect with reset cursor
              // This ensures we get all data from the beginning
              if (this.ws && this.ws.readyState === WebSocket.OPEN) {
                console.log(`[WsFeedClient] 🔄 Closing connection to reconnect with reset cursor`);
                
                // Temporarily disable auto-reconnect to prevent double reconnection
                ...
                // Reconnect after a short delay with reset cursor
                setTimeout(() => {
                  console.log(`[WsFeedClient] 🔄 Reconnecting with reset cursor (from_seq=1)`);
                  this._autoReconnect = wasAutoReconnect; // Restore auto-reconnect setting
                  this._closing = false;
                  this._explicitClose = false; // Ensure we can reconnect
                  this.resumeFromRequested = 1;
                  this.connect();
                }, 100);
              }
              
              return; // Don't process this init_begin, we're reconnecting
            }
